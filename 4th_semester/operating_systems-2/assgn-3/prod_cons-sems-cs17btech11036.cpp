...
sem_t empty, full, buff; // locks for producer, consumer and buffer
  ...
  for(int i = 0; i < rep_p; i++) {
    ...
    while(true) {
      ...
      if ((front_b == 0 && back_b == capacity - 1) || (back_b == (front_b - 1) % (capacity - 1))) { // buffer is full
        sem_post(&buff);; // lets a consumer consume.
        continue;
    ...
    sem_post(&buff); // so that the print statements are not jumbled
  ...
  for(int i = 0; i < rep_c; i++) {
    ...
    while(true) {
      ...
      if (front_b == -1) { // buffer is empty
        sem_post(&buff); // lets a producer consume.
        continue;
...
int main() {
  ...
  // open log file
  log_file.open("Sem-Prod_Cons-Log.txt");
  ...
  log_file<<fixed<<"Average waiting time for producers: "<<(double)waiting_time_prod / (n_p * rep_p)<<endl;
  log_file<<fixed<<"Average waiting time for consumers: "<<(double)waiting_time_cons / (n_c * rep_c)<<endl;
  log_file.close();
}
