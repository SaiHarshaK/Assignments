...
atomic<int>lock(0); // for compare and swap algo
lli waiting_time = 0;
lli worst_time = 0;
int n;
bool *waiting = NULL; // for a dynamic waiting array based on number of threads defined.
// output files
ofstream log_file;
...
void getInput(int* k, int* lam1, int* lam2) {
  ifstream input;
...
int main() {
  int k, lam1, lam2; // n is the number of threads, k is the frequency; t1, t2 will be delay values
                        // exponentially distributed with an average of lam1, lam2 seconds

  // process input
  getInput(&k, &lam1, &lam2);
  ...
  // open log file
  log_file.open("CAS-Bounded-Log.txt");
