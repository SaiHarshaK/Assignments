...
int main() {
  ...
  // open output file
  out_file.open("CLH.txt");
  if (!out_file.is_open()) {
    cerr<<"Error opening CLH log"<<'\n';
    exit( EXIT_FAILURE );
  }
  ...
  cout<<"CLH: Average time taken for thread to enter CS: "<<avg_entry / (n*k)<<'\n';
  cout<<"CLH: Average time taken for thread to exit CS: "<<avg_exit / (n*k)<<'\n';
