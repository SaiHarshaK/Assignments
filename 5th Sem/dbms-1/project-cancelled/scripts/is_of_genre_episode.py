...
tableName = "is_of_genre_episode"
header = ["episode_ID","media_ID", "value"]
isStr  = [1, 1, 1]
isMulti = [0, 0, 1] # for multivalued. if in dataset it is seperated by ','
toIgnore = [0,  0, 0] # 0 -> dont ignore, 1 is ignore (wrt to the header). useful when some fields are auto generated.
...
f = open("is_of_genre_episode.sql", "w+")
title = open("imdb/title.basics_episode.tsv", "r")

order_fill = [1,-1,-1,-1,-1,-1,-1,-1,3] # index 1. -1 -> ignore , from dataset
