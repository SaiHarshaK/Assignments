...
void cmp(struct Node** head_ref_1, struct Node** head_ref_2, int count_1, int count_2) {
  equalize_length(head_ref_1,head_ref_2,count_1,count_2);
  struct Node* l1 = *head_ref_1;
  struct Node* l2 = *head_ref_2;
...
int main(){
  int flag = 0, count_1=0, count_2=0;//flag for which DLL should store the digits of numbers.
  int flag_lz_1=0,flag_lz_2; //flag for leading zeroes of 1,2 to sanitize input
  int digit;
  ...
  while((digit = getchar()) != EOF) {
    if(digit == '\n') {
      cmp(&number_1,&number_2,count_1,count_2);
      delete_DLL(&number_1);
      delete_DLL(&number_2);
      ...
      if(flag == 0 ) {
        if(digit-48 == 0 && flag_lz_1 == 0) {
          continue;
        }
        else {
          flag_lz_1 = 1; //first non-zero - MSB for number 1
        }
        //input is sanitized, now create DLL of the number
        append(&number_1,digit-48);
        count_1++;
      }
      else {
        if(digit-48 == 0 && flag_lz_2 == 0) {
          continue;
        }
        else {
          flag_lz_2 = 1; //first non-zero - MSB for number 2
        }
        //input is sanitized, now create DLL of the number
