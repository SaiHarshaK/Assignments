...
__global__ void add(int *a, int *b, int *c) {
  long int total_set = ((N*N) / (NUM_BLOCKS*NUM_THREADS)) + 1;
  long int set = NUM_BLOCKS * NUM_THREADS;
  long int index_of_thread = threadIdx.x + (blockIdx.x * blockDim.x);

  for( long int i = 0; i < total_set; i++ ) {
    long int index = index_of_thread + (i*set);
    if( index < N*N ) {
